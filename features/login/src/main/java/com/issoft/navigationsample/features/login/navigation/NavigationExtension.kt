...
@Composable
private fun Screen.Splash.Render(
...
@Composable
private fun Screen.AppWalkThrough.Render(
    backStack: NavBackStack<NavKey>
) {
...
@Composable
private fun Screen.EnterEmail.Render(
    backStack: NavBackStack<NavKey>
) {
...
fun EntryProviderScope<NavKey>.installLoginGraph(backStack: NavBackStack<NavKey>){
    entry<Screen.Splash> { key -> key.Render(backStack) }
    entry<Screen.AppWalkThrough> { key -> key.Render(backStack) }
    entry<Screen.EnterEmail> { key -> key.Render(backStack) }
}